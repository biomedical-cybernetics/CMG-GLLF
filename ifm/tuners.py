...
class Linear_tuner(nn.Module):
    ...
    '''
    def __init__(self, indim, train_b, init_method, dataset, rectify=False, single_param=False):
        super().__init__()
        assert init_method in ['define','uniform']
        ...
        if single_param:
            ...
            n_params = indim // 3  # Assuming RGB (3 channels)
        else:
            n_params = indim
    ...
    def forward(self, x):
        ...
        if self.single_param:
            ...
            a_expanded = self.a.repeat(3)
            if self.train_b:
                b_expanded = self.b.repeat(3)
            else:
                b_expanded = self.b.repeat(3)
        else:
            a_expanded = self.a
...
class CM_GLLF_tuner(nn.Module):
    '''
    CM-GLLF function searching only in logistic range
    In logistic range, mu is in (0,0.5), I is in (0,1)
    '''
    def __init__(self, indim, train_I, init_method, dataset, rectify=False, offset=1e-6, single_param=False):
        super().__init__()
        assert init_method in ['define','uniform']
        if init_method == 'uniform':
            uniform_L, uniform_R = 0.125, 0.375
        # For colored images, we want one GLLF per pixel (shared across color channels)
        if single_param:
            n_params = 1  # Single parameter for all pixels
        elif dataset in ['CIFAR10','CIFAR100','SVHN']:
            n_params = indim // 3  # Assuming RGB (3 channels)
    ...
    def forward(self, x):
        ...
        if self.single_param:
            # Use the same parameters for all pixels
            mu_raw_expanded = self.mu_raw.expand(self.indim)
            if self.train_I:
                I_raw_expanded = self.I_raw.expand(self.indim)
            else:
                I_raw_expanded = self.I_raw.expand(self.indim)
        elif self.dataset in ['CIFAR10','CIFAR100','SVHN']:
            mu_raw_expanded = self.mu_raw.repeat(3)
            if self.train_I:
                I_raw_expanded = self.I_raw.repeat(3)
            else:
                I_raw_expanded = self.I_raw.repeat(3)
        else:
            mu_raw_expanded = self.mu_raw
            I_raw_expanded = self.I_raw
            
        # 将参数转换为(0,1)范围
        mu = 0.5 * torch.sigmoid(mu_raw_expanded)
        I = torch.sigmoid(I_raw_expanded)
        
        # 计算CM-GLLF输出，加上offset防止除零
        y = yL + (yR-yL) / (1 + (xmax-x)/(x-xmin+self.offset)*torch.exp((2-1/mu)*((x-xmin)/(xmax-xmin)-I)))
        return torch.clamp(y, min=0) if self.rectify else y
...
class CM_GLLF_tuner_valuewise(nn.Module):
    '''
    CM-GLLF function searching only in logistic range
    In logistic range, mu is in (0,0.5), I is in (0,1)
    '''
...
class CM_GLLF_tuner_valuewise_restrict(nn.Module):
    '''
    CM-GLLF function searching only in logistic range
    In logistic range, mu is in (0,0.5), I is in (0,1)
    '''
    def __init__(self, indim, train_I, init_method, dataset, minmax_setting, init_minmax=10, rectify=False, offset=1e-6, single_param=False):
        super().__init__()
        self.minmax_setting = minmax_setting
        self.init_minmax = init_minmax
        if init_method == 'uniform':
            uniform_L, uniform_R = 0.125, 0.375
        elif init_method == 'uniform_wide':
            uniform_L, uniform_R = 0.0, 0.5
        # For colored images, we want one GLLF per pixel (shared across color channels)
        if single_param:
            n_params = 1  # Single parameter for all pixels
        else:
        ...
        if init_method=='define':
            ...
            if train_I:
                ...
                init_I = torch.full((n_params,), 0.5)                  
        elif init_method=='uniMu_defI':
            init_mu = torch.empty((n_params,)).uniform_(0.125, 0.375)
            init_I = torch.full((n_params,), 0.5)   
        elif init_method=='uniI_defMu':
            ...
            init_mu = torch.normal(0.25, 0.05, (n_params,))
            init_mu = torch.clamp(init_mu, 1e-6, 0.5-1e-6)  # Clamp to valid range
            if train_I:
                init_I = torch.normal(0.5, 0.1, (n_params,))
                init_I = torch.clamp(init_I, 1e-6, 1-1e-6)  # Clamp to valid range
            else:
                init_I = torch.full((n_params,), 0.5)
        else:
    ...
    def forward(self, x):
        if self.minmax_setting=='old':
            xmax = x.detach().abs().max()
            xmin = -xmax
            yL = -1
            yR = 1                 
        elif self.minmax_setting=='minmax':
            xmin = x.detach().min()
            xmax = x.detach().max()
            yL = -1
            yR = 1
        elif self.minmax_setting=='minmax_mapping':
            xmin = x.detach().min()
            xmax = x.detach().max()
            yL = xmin
            yR = xmax
        elif self.minmax_setting=='abs':
            xmax = yR = float(self.init_minmax)
            xmin = yL = -xmax
            # add a mask to set those x values out of range to yL or yR
            mask_below_xmin = x < xmin
            mask_above_xmax = x > xmax
        else:
            raise ValueError(f"Invalid minmax_setting: {self.minmax_setting}. Must be 'old', 'minmax', or 'abs'.")
        ...
        # 计算CM-GLLF输出，加上offset防止除零
        y = yL + (yR-yL) / (1 + (xmax-x)/(x-xmin+self.offset)*torch.exp((2-1/mu)*((x-xmin)/(xmax-xmin)-I)))
        if self.minmax_setting=='abs':
            y = torch.where(mask_below_xmin, torch.full_like(y, yL), y)
            y = torch.where(mask_above_xmax, torch.full_like(y, yR), y)        
        return torch.clamp(y, min=0) if self.rectify else y


class CM_GLLF_tuner_clamp_valuewise(nn.Module):
    '''
    CM-GLLF function using direct clamping instead of logit transformation
    Based on CM_GLLF_logistic_clamp from neurons.py
    In logistic range, mu is in (0,0.5), I is in (0,1)
    Uses direct clamping to constrain parameters
    '''
    def __init__(self, indim, train_I, init_method, dataset, rectify=False, offset=1e-6, single_param=False):
        super().__init__()
        assert init_method in ['define','uniform','uniform_narrow','normal']
        
        # For colored images, we want one GLLF per pixel
        if single_param:
            n_params = 1  # Single parameter for all pixels
        else:
            n_params = indim
        ...
        if init_method == 'define':
            ...
            init_mu = torch.empty((n_params,)).uniform_(0.125, 0.375)
            init_I = torch.empty((n_params,)).uniform_(0, 1) if train_I else torch.full((n_params,), 0.5)
        # elif init_method == 'uniform_narrow':
        #     init_mu = torch.empty((n_params,)).uniform_(0.125, 0.375)
        #     init_I = torch.empty((n_params,)).uniform_(0.25, 0.75) if train_I else torch.full((n_params,), 0.5)
        elif init_method == 'normal':
            init_mu = torch.normal(0.25, 0.05, (n_params,))
            init_mu = torch.clamp(init_mu, 1e-6, 0.5-1e-6)  # Clamp to valid range
            if train_I:
                init_I = torch.normal(0.5, 0.1, (n_params,))
                init_I = torch.clamp(init_I, 1e-6, 1.0-1e-6)  # Clamp to valid range
            else:
                init_I = torch.full((n_params,), 0.5)
        ...
        # Initialize learnable parameters - directly store mu and I without transformation
        self.mu_raw = nn.Parameter(init_mu)
        self.mu_raw.is_special_param = True
        
        if self.train_I:
            self.I_raw = nn.Parameter(init_I)
            self.I_raw.is_special_param = True
        else:
            self.register_buffer('I_raw', init_I)
    ...
    def forward(self, x):
        ...
        mu = torch.clamp(mu_raw_expanded, min=1e-6, max=0.5-1e-6)  # mu in (0, 0.5)
        I = torch.clamp(I_raw_expanded, min=1e-6, max=1.0-1e-6)   # I in (0, 1)
...
class CM_GLLF_tuner_trainable_minmax(nn.Module):
    '''
    CM-GLLF function with trainable mu, I, xmin, xmax, yL, yR parameters
    All min/max parameters are initialized using dataset-specific values
    Input values outside [xmin, xmax] are set to 0 with zero gradients
    '''
    def __init__(self, indim, train_I, init_method, dataset, mode='all', rectify=False, offset=1e-6):
        super().__init__()
        self.mode = mode
        self.offset = offset
        self.dataset = dataset
        ...
        # Initialize mu and I parameters
        if init_method == 'define':
            if mode == 'logistic':
                init_mu = torch.full((n_params,), 0.25)
            elif mode == 'logit':
        ...
        # Initialize min/max parameters using dataset-specific values
        if dataset == 'CIFAR10':
            init_xmin = min([-0.4914/0.2470, -0.4822/0.2435, -0.4465/0.2616])
            init_xmax = max([(1-0.4914)/0.2470, (1-0.4822)/0.2435, (1-0.4465)/0.2616])
            init_yL = init_xmin
            init_yR = init_xmax
        elif dataset == 'CIFAR100':
            init_xmin = min([-0.5071/0.2675, -0.4867/0.2563, -0.4409/0.2761])
            init_xmax = max([(1-0.5071)/0.2675, (1-0.4867)/0.2563, (1-0.4409)/0.2761])
            init_yL = init_xmin
            init_yR = init_xmax
        else:
        ...
        # Initialize learnable parameters for mu and I
        if mode == 'logistic':
            # For logistic mode, constrain mu to (0, 0.5)
            self.mu_raw = nn.Parameter(torch.logit(2 * init_mu))
        elif mode == 'logit':
        ...
        self.xmin = nn.Parameter(torch.full((n_params,), init_xmin))  # Will map to init_xmin
        self.xmax = nn.Parameter(torch.full((n_params,), init_xmax))  # Will map to init_xmax
        self.yL = nn.Parameter(torch.full((n_params,), init_yL))    # Will map to init_yL
        self.yR = nn.Parameter(torch.full((n_params,), init_yR))    # Will map to init_yR
        
        self.xmin.is_special_param = True
        self.xmax.is_special_param = True
        self.yL.is_special_param = True
        self.yR.is_special_param = True
    ...
    def forward(self, x):
        ...
        # Use the trainable min/max compute_gllf function
        return compute_gllf_trainable_minmax.apply(x, mu, I, self.xmin, self.xmax, self.yL, self.yR,
                                                   self.rectify, self.mode, self.offset)
